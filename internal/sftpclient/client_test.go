...
func TestSFTPClient_FullCoverage(t *testing.T) {
	...
	_ = os.WriteFile(testFile, []byte("1234567890"), 0644)
...
func TestSFTPClientUpload_AdvancedNetwork(t *testing.T) {
	...
	defer os.RemoveAll(tempDir)
	testFile := filepath.Join(tempDir, "test.txt")
	_ = os.WriteFile(testFile, []byte("large data chunk for testing network issues"), 0644)
...
func TestSFTPClient_UploadResumeMismatch(t *testing.T) {
	...
	_ = os.WriteFile(testFile, localContent, 0644)
