document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderLocalFiles = () => {
        // ⚡ Bolt: Filter files before sorting to improve performance. Conditionally bypass filter if empty.
        // 📊 Impact: O(n log n) sorting now only runs on the matching files. Bypasses redundant O(N) array allocation when filter is empty.
    ...
    const renderRemoteFiles = () => {
        // ⚡ Bolt: Conditionally bypass filter if empty to improve performance.
        // 📊 Impact: Bypasses redundant O(N) array allocation when filter is empty.
