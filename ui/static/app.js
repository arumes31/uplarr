    ...
    const sortFiles = (files, sortKey, sortDir) => {
        ...
        sorted.sort((a, b) => {
            ...
            switch (sortKey) {
                ...
                case 'type': {
                    // ⚡ Bolt: Replace object-instantiating .split().pop() with allocation-free lastIndexOf
                    // 📊 Impact: Significant reduction in GC pressure and speedup for O(N log N) sorting operations
                    const idxA = a.name.lastIndexOf('.');
                    const extA = idxA !== -1 ? a.name.substring(idxA + 1).toLowerCase() : '';
                    const idxB = b.name.lastIndexOf('.');
                    const extB = idxB !== -1 ? b.name.substring(idxB + 1).toLowerCase() : '';
