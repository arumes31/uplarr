...
## 2024-05-25 - [Optimize List Selection Performance]
**Learning:** Re-querying the DOM and finding the index of an element in a long list on every interaction causes noticeable UI lag (O(N) operation).
**Action:** Assign data attributes during list generation to track indices and perform O(1) lookups on interaction, deferring expensive DOM queries only to the paths that strictly require them (like bulk shift-selection).
